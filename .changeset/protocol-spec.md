...
appendices, and each section maps to the conformance assertion set.
`OLOS_SPEC_STATUS` is now `draft-v1.0.0`. The new
`OLOS_CONFORMANCE_SPEC_REFS` export maps every conformance assertion ID to
the spec section that specifies it.
