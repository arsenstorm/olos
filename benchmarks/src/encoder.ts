...
export function spawnEncoder(options: EncoderOptions): ChildProcess {
  return spawn("ffmpeg", encoderArgs(options), {
    stdio: ["pipe", "inherit", "inherit"],
  });
}

function encoderArgs(options: EncoderOptions): string[] {
  return [...inputArgs(options), ...outputArgs(options)];
}

// Raw RGBA frames on stdin at the bench frame rate.
function inputArgs(options: EncoderOptions): string[] {
  return [
    "-hide_banner",
    "-loglevel",
    "error",
...
// libx264 zero-latency encode into an LL-HLS fMP4 segmenter.
function outputArgs(options: EncoderOptions): string[] {
  return [
    "-an",
    "-c:v",
    "libx264",
